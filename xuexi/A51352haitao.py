...
def work_task01():
    ...
    evo_conn = pymysql.connect(host="172.31.239.72", user="root", password="root123", charset="utf8")
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no2 = random.choice(b)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no3 = random.choice(c)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no4 = random.choice(d)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no5 = random.choice(e)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no6 = random.choice(f)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no7 = random.choice(g)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no8 = random.choice(h)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no9 = random.choice(j)[10]
    evo_cursor = evo_conn.cursor()
    evo_cursor.execute(
        'SELECT c.bucket_code,c.lot_id,c.sku_code,c.sku_name,lot.lot_att02,lot.lot_att04,lot.lot_att05,lot.lot_att06,lot.lot_att07,lot.lot_att08,lot.lot_att09 FROM evo_wes_inventory.lot AS lot RIGHT JOIN (SELECT a.lot_id,a.bucket_code,b.sku_code,b.sku_name FROM evo_wes_inventory.level3_inventory as a LEFT JOIN evo_wes_basic.basic_sku as b ON a.sku_id=b.id WHERE a.zone_code = "vip" AND a.quantity > 2) AS c ON lot.id=c.lot_id ORDER BY RAND() LIMIT 1;')
    ...
    inv_lot_no10 = random.choice(k)[10]
...
</root>'.format(time_stamp, time_stamp, time1, time_stamp, sku_code1, sku_name1, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality, mfg_date, exp_date, inv_type, vendor_code, po_no,inv_lot_no,
                time_stamp, time_stamp, time1, time_stamp, sku_code2, sku_name2, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp,time_stamp, quality2, mfg_date2, exp_date2, inv_type2, vendor_code2, po_no2,inv_lot_no2,
                time_stamp, time_stamp, time1, time_stamp, sku_code10, sku_name10, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality10, mfg_date10, exp_date10, inv_type10, vendor_code10, po_no10,inv_lot_no10,
                time_stamp, time_stamp, time1, time_stamp, sku_code3, sku_name3, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality3, mfg_date3, exp_date3, inv_type3, vendor_code3, po_no3,inv_lot_no3,
                time_stamp, time_stamp, time1, time_stamp, sku_code4, sku_name4, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality4, mfg_date4, exp_date4, inv_type4, vendor_code4, po_no4,inv_lot_no4,
                time_stamp, time_stamp, time1, time_stamp, sku_code5, sku_name5, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality5, mfg_date5, exp_date5, inv_type5, vendor_code5, po_no5,inv_lot_no5,
                time_stamp, time_stamp, time1, time_stamp, sku_code6, sku_name6, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality6, mfg_date6, exp_date6, inv_type6, vendor_code6, po_no6,inv_lot_no6,
                time_stamp, time_stamp, time1, time_stamp, sku_code7, sku_name7, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality7, mfg_date7, exp_date7, inv_type7, vendor_code7, po_no7,inv_lot_no7,
                time_stamp, time_stamp, time1, time_stamp, sku_code8, sku_name8, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality8, mfg_date8, exp_date8, inv_type8, vendor_code8, po_no8,inv_lot_no8,
                time_stamp, time_stamp, time1, time_stamp, sku_code9, sku_name9, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, time_stamp, quality9, mfg_date9, exp_date9, inv_type9, vendor_code9, po_no9,inv_lot_no9))
'''
#工作提供握手接口
...
if __name__ == '__main__':
    app.run(host = '172.31.246.140',port = 1234,debug = True)